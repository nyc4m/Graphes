...
public class Dijkstra {
    ...
    /**
     * Le tableau contenant la liste des sommets. Ce tableau sert principalement
     * aux tests de l'algorithme, il n'a pas de réelle utilité pour l'execution.
     */
    ...
    /**
     * Constructeur de dijkstra permet principalement d'initialiser toutes les
     * listes, et de cloner le graphe concerné
     *
     * @param g
     */
    ...
    /**
     * Cette méthode sert à "représenter" la valeur +OO. Elle correspond à
     * l'addition de l'ensemble des poids des arêtes +1. Il est impossible de
     * trouver une distance plus longue
     *
    ...
    /**
     * Méthode permettant d'intialiser tous les tableaux concernés par
     * l'algorithme
     */
    public void intitialisation(int dep, int soleil) {
        /**
         * Cette partie sert à attribuer une valeur -qui ne gênera pas
         * l'algorithme- à l'indice 0 de tous les tableaux. Ainsi, lorsqu'une
         * valeur sera entrée dans un tableau, elle sera entrée à l'indice
         * correspondant au sommet à laquelle elle est associée. exemple : la
         * valeur entrée à l'indice 3 du tableau distances correspond à la
         * distance minimum pour aller au sommet 3.
         */
        //////////////////////////////////
...
/**
 * Méthode permettant d'isoler les zombies et les shadocks
 * @param positionZ Position des zombies
 * @param temp Position des sommets interdits (shadocks)
 */
    public void isolerZombie(int positionZ, ArrayList<Integer> temp) {
    ...
    /**
     * Méthode permettant d'afficher de manière brute l'état de tous les
     * tableau. C'est l'unique fois ou le tableau sommet est utilisé, pour
     * associer chaque valeur à un sommet.
     */
    ...
    /**
     * Méthode permettant de savoir si tous les sommets ont été traité. pour ce
     * faire, tous le tableau mark est parcouru. Si le tableau contient un
     * "false" c'est que tous les sommets n'ont pas été traité.
     *
     * @return Retourne faux si tous les sommets n'ont pas été traité, et vrai
     * si tous les sommets ont été traités.
     */
    ...
    /**
     * Méthode l'indice de la distance minimum du tableau distance, parmis les
     * sommets non traités.
     *
     * @return renvoi l'indice du tableau distances contenant la distance la
     * plus courte.
     */
    public int minDistance() {
        ...
        for (int i = 1; i < this.distances.size(); i++) { //tous le tableau des distances est parcouru pour trouver la distance minimum.
            /**
             * A chaque itération, on effectue un test : si la valeur contenu à
             * l'indice i du tableau est inférieur à valMin ET QUE LE SOMMET
             * ASSOCIE N'A JAMAIS ETE TRAITE alors on remplace la valeur minimum
             * dans valMin et on stocke son indice associé.
             */
    ...
    /**
     * Méthode permettant de connaitre l'indice de la valeur minimum du tableau
     * distance. Elle servait pour l'ancienne version de minDistance.
     *
    ...
    /**
     * Méthode permettant de mettre à jour le tableau des distances et des
     * antécédents chaque fois qu'un chemin plus court est trouvé.
     *
    ...
    public void maj_distance(int a, int b) {
        ...
        /**
         * Si la distance pour aller au sommet B est plus longue que la distance
         * de A jusqu'à B ET Que la distance AB n'est pas de 0 (qui
         * correspondrait à deux sommets A et B qui ne sont pas liés) ET Que le
         * sommet B n'a pas déjà été traité.
         *
         * ALORS on met à jour la distance pour aller à B en ajoutant la
         * distance de A.
         *
         */
     ...
     * @param soleil le sommet d'arrivée
     */
    ...
    /**
     * Méthode permeetant de faire un plus court chemin pour les licornes
     * @param dep Le sommet de départ
     * @param soleil La position du soleil
     * @param positionZ La position du vaisseau zoombie
     * @param sommetShad  Tableau conteant les sommet ou les shadock sont à proximité
     */
    public void plusCourtChemin(int dep, int soleil, int positionZ, ArrayList<Integer> sommetShad) {
        this.intitialisation(dep, soleil);
        this.isolerZombie(positionZ,sommetShad);
        while (!this.listeTerminee()) {
            int x = this.minDistance();
            this.mark.set(x, Boolean.TRUE);
            for (int i = 1; i <= this.graphe.getNbSommet(); i++) {
...
/**
 * Méthode permettant de déterminer ou le vaisseau shadock à le droit de se déplacer
 * @param sommet Le sommet de la planéte shdock
 * @param soleil Position du soleil
 * @return Un tableau contenant toutes les distances autorisées
 */
    ...
    /**
     * Genere un tableau contenant "l'itineraire" le plus court pour aller de
     * sommetDep a sommet
     *
...
/**
 * Méthode permettant d'afficher le chemin
 * @param sommetDep Sommet de départ
 * @param sommetArr Sommet d'arrivée
 * @return Chaine de caractére comprenant les valeurs
 */
